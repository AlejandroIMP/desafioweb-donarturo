...
import { createProduct, getProducts, getProductById, updateProduct, updateProductState } from "../controllers/products.controller";
import { verifyToken, verifyRol } from "../middleware/auth";
import { ROLES } from "../config/roles";

const router = Router();

router.get('/productos', verifyToken, verifyRol([ROLES.ADMIN, ROLES.USER, ROLES.CLIENTE]), getProducts);
